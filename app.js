...
function handleServiceForm(e) {
  ...
  state.services.push({ id: crypto.randomUUID(), name: data.get('name').trim(), start: data.get('start'), end: data.get('end') });
  saveState();
  updateDropdowns();
  renderServices();
  serviceForm.reset();
...
function handleFunctionForm(e) {
  ...
  state.functions.push({ id: crypto.randomUUID(), name: data.get('name').trim(), serviceIds });
  saveState();
  updateDropdowns();
  renderFunctions();
  functionForm.reset();
...
function handleEmploymentForm(e) {
  ...
  state.employment.push({ id: crypto.randomUUID(), percent: Number(data.get('percent')), hours: Number(data.get('hours')) });
  saveState();
  updateDropdowns();
  renderEmployment();
